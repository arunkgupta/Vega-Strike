...
def AssignMission ():
    ...
    fixers.CreateChoiceButtons(Base.GetCurRoom(),[
            fixers.Choice("bases/fixers/yes.spr","bases/fixers/iso_antagonist_mission.py",_("Accept This Agreement")),
            fixers.Choice("bases/fixers/no.spr","bases/fixers/iso_antagonist_decline.py",_("Decline This Agreement"))])
...
if (not Director.getSaveDataLength(playernum,"iso_mission1")):
    ...
    if (fixers.checkSaveValue(playernum,"iso_mission1",0)):
        ...
        if (VS.numActiveMissions()>1):
            Base.Message (_("your other contracts interfere with me assigning you a new mission. Finish them, then talk to me."))
            ...
            Base.Message(_("Now the stakes are higher. My elite force had planned to assault a ISO stronghold in the Defiance system, but we did not anticipate the number of ISO ships we found to be there. Therefore, we will send in a larger ship that will assist you in destroying the last vestiges of the only truly substantial ISO defense force in any system.  Once this is secured, this system, and all others like it, will be more amenable to control via the more subtle applications of IntelSec diplomacy. Your assistance is worth 40000 credits to us."))
            AssignMission()

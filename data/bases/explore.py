...
def exploremission(part,filename,sysfile,creditnum):
    ...
    if part==1:
        fixers.CreateChoiceButtons(Base.GetCurRoom(),[
            fixers.Choice("bases/fixers/yes.spr",filename,_("Accept This Mission")),
            fixers.Choice("bases/fixers/no.spr","bases/fixers/no.py",_("Decline This Mission"))])
        Base.Message(_("""Hi. Our researchers have found a new signal similar to that of a jump point coming from an area in the
...
We hope that you will accept this mission. You will earn %.2f credits when you explore the area.""")%(sysfile.split('/')[-1],creditnum))
        ...
        Base.Message(_("""Excellent! Go to the unknown energy source that we have uploaded to your ship computer
...
into the energy source and get close to all of the planets in the connecting system.""")%sysfile.split('/')[-1])
        Base.EnqueueMessage(_("""You will earn %.2f credits when this is completed""")%creditnum)
        plyr=VS.getPlayer().isPlayerStarship()
        ...
        Base.Message(_("""Go to the unknown energy source that we have uploaded to your ship computer
as a navpoint in #55ffff%s#000000. Once next to the energy source, jump
into the energy source and get close to all of the planets in the connecting system.""")%sysfile.split('/')[-1])
